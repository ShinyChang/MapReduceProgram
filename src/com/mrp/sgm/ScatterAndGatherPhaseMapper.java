...
public class ScatterAndGatherPhaseMapper extends DefaultMapper<DoubleTextPair> {
	...
	protected void doFirstPhaseOutput(Context context, Text value) throws IOException, InterruptedException {
		...
		outputKey = new DoubleTextPair(columnValue[0], columnValue[1]);

		// 如果有欄位被選取(僅適用SSB)
		if (columnValue.length > 2) {
			outputValue.set(columnValue[2]);
		}
	...
	protected void doDimensionTable(Context context, Text value) throws IOException, InterruptedException {
		...
		outputKey = new DoubleTextPair(new IntWritable(Integer.parseInt(columnValue[0])), new IntWritable(tableIndex));
